...
module namespace display="http://www.xqdoc.org/1.0/display";
...
declare function display:print-intro($local as xs:boolean) as element()+
{
  ...
  Visit <a href="http://xqdoc.org">xqdoc.org</a> to find the latest developments for this XQuery
  open source tool.</p>
  </div>
  )
};
...
declare function display:print-module-control($uri as xs:string, $local as xs:boolean) as item()+ 
{
  ( display:print-module($uri, $local),
    ...
    "&#160;",
    display:print-namespaces($uri, $local),
    "&#160;",
...
declare function display:print-variables($uri as xs:string, $local as xs:boolean) as element()* 
{
  if (fn:exists(fn:collection($display:XQDOC_COLLECTION)/xq:xqdoc[xq:module/xq:uri = $uri]/xq:variables/xq:variable)) then
      (<h4>Variables</h4>, 
       <div id="variables">
       {
            for $v in fn:collection($display:XQDOC_COLLECTION)/xq:xqdoc[xq:module/xq:uri = $uri]/xq:variables/xq:variable
            return 
                <div>
...
(:~
 ...
 :  @return HTML.
 :)
declare function display:print-namespaces($uri as xs:string, $local as xs:boolean) as element()* 
{
  if (fn:exists(fn:collection($display:XQDOC_COLLECTION)/xq:xqdoc[xq:module/xq:uri = $uri]/xq:namespaces/xq:namespace)) then
      (<h4>Namesapces</h4>, 
       <div id="namespaces">
       {
            for $v in fn:collection($display:XQDOC_COLLECTION)/xq:xqdoc[xq:module/xq:uri = $uri]/xq:namespaces/xq:namespace[not(data(@uri) = (fn:collection($display:XQDOC_COLLECTION)/xq:xqdoc[xq:module/xq:uri = $uri]/(./xq:module/xq:uri, ./xq:imports//xq:uri)/text(), "http://www.w3.org/2012/xquery"))]
            return 
                <div>
                 <a name="{ xs:string($v/@uri) }"/>
                 <ul class="method">
                   <li class="left">{ xs:string($v/@uri) }</li>
                   <li class="right">{ $v/xq:comment/xq:description/node() }</li>
                 </ul>
                 <ul>
                 { display:print-comment($v/xq:comment/xq:since, "Since:", $local) }
...
declare function display:print-signature($sigs as xs:string*, $local as xs:boolean) as item()*
{
  ...
  return
  (let $annotations := fn:tokenize(fn:substring-before($sig, " function "), "%")
   let $params := fn:substring-after(fn:substring-after($sig, " function "), "(")
   ...
   return
     (if ($cnt > 1) then 
	  ("OR", <br/>,<br/>)
        ...
        (),
      for $annotation at $index in $annotations
      return (
        if ($index = 1) then ()
        else concat('%', $annotation),<br/>),
     ...
     for $i in $comment
       return           
             if (fn:not(fn:exists($name))) then 
               <p>{display:newline-to-br($i/node())}</p>     
       ...
       <ul>
          {
            ...
            return
              if ($name = "Parameters:") then
                ...
                <li>{display:newline-to-br($i/node())}</li>
          }
      </ul>) 
    else
      <p>{display:newline-to-br($comment/node())}</p> 
  else
    ()  
};

declare function display:newline-to-br($n as node()*) as node()*
{
    parse-xml-fragment(replace(serialize($n), '([^>])&#x0a;', '$1<br/>')
                          => replace('xmlns[^=]+="[^"]+"', ''))
};
...
declare function display:print-comment-param($entry as element()) as item()* 
{
    ...
    let $aft :=  fn:substring-after(fn:normalize-space(replace(serialize($tmp), '([^>])&#x0a;', '$1<br/>')), " ")
    return
       (if (fn:string-length($bef) > 0) then
		parse-xml-fragment(fn:concat($bef, " - ", $aft))
	  else
		$tmp,
...
(: BaseX specific functions to create the cache collection and fill it :)

(:~  
 :  Read all xq* files in target into the cache collection as xqdoc XMLs
 :
 :  @param target the root of the file tree to traverse
 :  @author Omar Siam
 :)

declare %updating function display:cache-xqdocs($target as xs:string) {
  let $target := if (ends-with($target, '/')) then $target else $target ||'/' 
  for $module in file:list($target, true())[display:xquery-resource(.)]
     return db:replace($display:XQDOC_COLLECTION, $module, inspect:xqdoc(file:resolve-path($module, $target)))
};

(:~ 
 :  Defines possible file extensions to look at
 :  @author Omar Siam
 :)

declare %private function display:xquery-resource($baseuri as xs:string)
as xs:boolean {
